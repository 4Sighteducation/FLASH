-- COMPLETE BATCH IMPORT FOR ALL EXAM BOARDS
-- Run each section one at a time to avoid timeouts

-- SECTION 1: EDEXCEL A-LEVEL
-- 1A: Modules
...
WHERE ct.exam_board = 'EDEXCEL' AND ct.exam_type = 'A_LEVEL'
...
-- Check EDEXCEL progress
SELECT 'EDEXCEL A-Level complete', topic_level, COUNT(*) 
FROM curriculum_topics ct
JOIN exam_board_subjects ebs ON ct.exam_board_subject_id = ebs.id
JOIN exam_boards eb ON ebs.exam_board_id = eb.id
WHERE eb.code = 'EDEXCEL'
GROUP BY topic_level;

-- SECTION 2: OCR A-LEVEL (repeat same pattern)
-- Copy the above 3 INSERT statements and change 'EDEXCEL' to 'OCR'

-- SECTION 3: WJEC A-LEVEL
-- Copy the above 3 INSERT statements and change 'EDEXCEL' to 'WJEC'

-- SECTION 4: GCSE TOPICS (All exam boards)
-- After A-Levels are done, repeat the same pattern for GCSE
-- Change ct.exam_type = 'A_LEVEL' to ct.exam_type = 'GCSE'
