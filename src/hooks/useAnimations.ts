...
export const usePulseAnimation = (enabled: boolean = true) => {
  ...
  useEffect(() => {
    ...
    return () => pulse.stop();
  }, [enabled, effects.animations.pulseEnabled, effects.animations.pulseScale, effects.animations.pulseDuration, scaleAnim]);
...
export const useFloatAnimation = (enabled: boolean = true) => {
  ...
  useEffect(() => {
    ...
    return () => float.stop();
  }, [enabled, effects.animations.floatEnabled, effects.animations.floatDistance, translateY]);
...
export const useGlowAnimation = (enabled: boolean = true) => {
  ...
  useEffect(() => {
    ...
    return () => glow.stop();
  }, [enabled, effects.animations.glowEnabled, effects.animations.glowIntensity, opacity]);
...
export const useFadeIn = (delay: number = 0) => {
  ...
  }, [delay, effects.timing.medium, opacity, translateY]);
...
export const useShimmerAnimation = (width: number = 200) => {
  const validWidth = Math.max(50, width); // Ensure minimum width
  const translateX = useRef(new Animated.Value(-validWidth)).current;
...
export const useStaggerAnimation = (itemCount: number, staggerDelay: number = 50) => {
  ...
  }, [animations, staggerDelay, effects.timing.medium]);
